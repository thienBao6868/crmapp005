	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="container-fluid">
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="progress">
										<div class="progress-bar progress-bar-danger"
											...
											aria-valuemax="100"
											style="width:${percentOfTask.chuaThucHien} "></div>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="progress">
										<div class="progress-bar progress-bar-megna"
											role="progressbar" aria-valuenow="40" aria-valuemin="0"
											aria-valuemax="100"
											style="width:${percentOfTask.dangThucHien}"></div>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="progress">
										<div class="progress-bar progress-bar-primary"
											role="progressbar" aria-valuenow="40" aria-valuemin="0"
											aria-valuemax="100" style="width: ${percentOfTask.hoanThanh}">
										</div>
									</div>
								</div>
							</div>
				...
				<c:forEach items="${listUserOfProject}" var="user">
					<div class="row">
						<div class="col-xs-12">
						...
						<div class="col-md-4">
							<div class="white-box">
								...
								<c:forEach items="${listAllTaskOfProject}" var="task">
									<c:if test="${task.status.id == 1 && task.user.id == user.id}">
										<a href="task-detail?id_task=${task.id}">
												<div class="mail-contnet">
													<h5>${task.name}</h5>
													<span class="mail-desc"></span> <span class="time">Bắt
														đầu: ${task.start_date}</span><br/> <span class="time">Kết
														thúc: ${task.end_date}</span>
												</div>
											</a>
									</c:if>
						...
						<div class="col-md-4">
							<div class="white-box">
								...
								<c:forEach items="${listAllTaskOfProject}" var="task">
									<c:if test="${task.status.id == 2 && task.user.id == user.id}">
										<a href="task-detail?id_task=${task.id}">
												<div class="mail-contnet">
													<h5>${task.name}</h5>
													<span class="mail-desc"></span> <span class="time">Bắt
						...
						<div class="col-md-4">
							<div class="white-box">
								...
								<c:forEach items="${listAllTaskOfProject}" var="task">
									<c:if test="${task.status.id == 3 && task.user.id == user.id}">
										<a href="task-detail?id_task=${task.id}">
												<div class="mail-contnet">
													<h5>${task.name}</h5>
													<span class="mail-desc"></span> <span class="time">Bắt
