			...
			<div class="container-fluid">
				...
				<div class="row">
					...
					<div class="col-md-8 col-xs-12">
						<div class="white-box">
							<form class="form-horizontal form-material" id="form-save"
								id-task="${task.id}">
										...
										<select class="form-control form-control-line" name="tenDuAn">
											<c:forEach items="${listProject}" var="project">
												<option value="${project.id}"
													${task.project.id == project.id ? 'selected': ''}>${project.name}</option>
											</c:forEach>
										</select>
									</div>
								</div>
										...
										<select class="form-control form-control-line"
											...
											<c:forEach items="${listUser}" var="user">
												<option value="${user.id}"
													${user.id == task.user.id ? 'selected' : '' }>${user.fullname}</option>
											</c:forEach>
										</select>
									</div>
								</div>
										...
										<select class="form-control form-control-line"
											...
											<c:forEach items="${listStatus}" var="status">
												<option value="${status.id}"
													${status.id == task.status.id ? 'selected' : ''}>${status.name}</option>
											</c:forEach>
										</select>
									</div>
								</div>
